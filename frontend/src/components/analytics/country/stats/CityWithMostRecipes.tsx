...
export default function CityWithMostRecipes({ posts, selectedCountry }) {
    ...
    const includedPosts = selectedCountry ? filteredPostsByCountry(posts, selectedCountry) : posts 
    ...
    const maxValue = Object.values(recipeCountByCity).reduce((max, score) => (score > max ? score : max), -Infinity);
